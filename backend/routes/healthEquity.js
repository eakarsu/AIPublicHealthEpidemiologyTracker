...
router.get('/', async (req, res) => {
  if (req.query.format === 'csv') {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="health_equity.csv"`);
      return res.send(csv);
  ...
  try {
    ...
    const [rows, countResult] = await Promise.all([
      pool.query('SELECT * FROM health_equity ORDER BY report_date DESC LIMIT $1 OFFSET $2', [limit, offset]),
      pool.query('SELECT COUNT(*) FROM health_equity'),
    ]);
...
router.post('/:id/ai-analyze', auth, aiRateLimiter, async (req, res) => {
  try {
    ...
    await persistAI(req.user?.id, 'health_equity/ai', {}, analysis);
    res.json({ analysis, equity: item });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.post('/ai/disparity-analysis', auth, aiRateLimiter, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM health_equity ORDER BY report_date DESC');
    const analysis = await queryAI(
