// AI Nowcast case trajectory
// Real-time ML model predicting case counts 7/14 days ahead
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPublicHealthEpidemiologyTracker'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIPublicHealthEpidemiologyTracker. Focus area: Nowcast case trajectory. ${`Real-time ML model predicting case counts 7/14 days ahead`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Nowcast case trajectory.\n${`Real-time ML model predicting case counts 7/14 days ahead`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'nowcast', context, parsed);
    res.json({ feature: 'nowcast', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[nowcast]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['nowcast']);
